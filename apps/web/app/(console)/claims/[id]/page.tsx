...
export default function ClaimDetailPage() {
  ...
  return <div className="page">
    ...
    <div className="content-grid">
      ...
      <aside className="stack"><section className="card section-card"><div className="section-heading"><h2>Policy context</h2></div><div className="section-body compact-stack"><div><span className="muted">Policy</span><strong style={{ display: "block" }}>{policy.data?.policy_number ?? "Loading…"}</strong></div><div><span className="muted">Coverage</span><strong style={{ display: "block" }}>{policy.data ? `${new Date(policy.data.start_date).toLocaleDateString()} – ${new Date(policy.data.end_date).toLocaleDateString()}` : "—"}</strong></div></div></section><section className="card section-card"><div className="section-heading"><h2>Phase boundary</h2></div><div className="section-body"><div className="notice"><FileSearch size={20} aria-hidden="true" /><span>Phase 1 provides experimental visible damage and part findings inside each submitted inspection. Risk, fraud, baseline comparison, and repair estimates remain unavailable.</span></div></div></section></aside>
    </div>
  </div>;
}
